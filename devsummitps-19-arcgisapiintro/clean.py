...
group_list = gis.groups.search("owner:dmartinez")
print("")
...
try:
    for user in user_list:
        ...
        if user.fullName in ['Admin', 'Esri', 'Esri Navigation']:
            print('skipped')
            continue
...
for user in user_list:
    if user.username == "dmart" or user.username.startswith("esri_") or user.username.startswith("AVWORLD"):
        continue
    else:
